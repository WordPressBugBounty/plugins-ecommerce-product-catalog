...
jQuery( document ).ready(
	function () {
		...
		jQuery( document ).on(
			'change',
			'#shopping-cart-container .edit-product-quantity',
			function (e) {
				var product_count    = 0;
				...
				jQuery( 'input.edit-product-quantity' ).each(
					function () {
		...
		jQuery( document ).on(
			'change',
			"#shopping-cart-container .variation_select",
			function () {
				var swicher    = jQuery( this );
				...
				swicher.closest( 'td.td-name' ).find( '.variation_select' ).each(
					function () {
...
function apply_updated_products_table(response, data) {
	jQuery( "table.cart-products" ).remove();
	if (data['raw'] === 1) {
		jQuery( "#shopping-cart-submit-container .ic-form > form" ).prepend( response ).append(
			function () {
