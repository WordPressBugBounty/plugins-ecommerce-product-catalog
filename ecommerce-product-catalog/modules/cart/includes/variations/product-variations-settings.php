...
function product_variations_menu() {
	...
	<a id="product-variations-settings" class="element"
		href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=attributes-settings&submenu=product-variations' ); ?>"><?php _e( 'Product Variations', 'ecommerce-product-catalog' ); ?> </a>
		<?php
}
				...
				if ( $product_variations_settings['count'] > 0 ) {
					...
					if ( ic_is_variations_price_effect_active() ) {
						implecode_settings_radio(
							__( 'Variation price info', 'ecommerce-product-catalog' ),
							'product_variations_settings[info]',
							$product_variations_settings['info'],
							array(
