...
class ic_cart_variations_selectors {
	...
	function get_price_effect_dropdown_info( $mod_type, $price_effect, $product_price, $product_id ) {
		...
		} elseif ( is_ic_any_variation_price_effect( $product_id ) && ! ic_has_multiple_product_variations( $product_id ) && $settings['info'] != 'price-effect' && $settings['info'] != 'no-info' && ic_is_variations_price_effect_active() ) {
			$mod = ' (' . price_format( $product_price ) . ')';
			$mod = str_replace( '()', '', $mod );
		} else {
			$mod = '';
