...
jQuery( document ).ready(
	function () {
		jQuery( ".product-entry #product_details .variation_select, .product-entry .ic-variations-shortcode .variation_select" ).on(
			'change',
			function () {
				...
				if (value !== '') {
					...
					dropdown.closest( '.variations-container' ).find( '.variation_select' ).each(
						function () {
							var this_dropdown = jQuery( this );
							if (var_num === 1) {
								this_dropdown.find( "option:disabled" ).each(
									function () {
					...
					if (all_selected === 1) {
						...
						jQuery.when( ic_get_variation_details( fields, values, product_id, variation_id, var_lp ) ).done(
							function (out) {
								out = JSON.parse( out );
								if (out === null) {
									return false;
		...
		jQuery( ".product-entry #product_details .variation_select" ).each(
			function () {
		...
		jQuery( document ).on(
			'change',
			".cart-products .variation_select",
			function () {
