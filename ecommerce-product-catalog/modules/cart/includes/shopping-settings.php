...
function shopping_cart_menu() {
	...
	<a id="shopping-cart-settings" class="element"
		href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=shopping-cart' ); ?>"><?php echo apply_filters( 'ic_simple_cart_settings_menu_name', __( 'Shopping Cart', 'ecommerce-product-catalog' ) ); ?> </a>
		<?php
}
			...
			<form method="post" action="options.php">
				...
				<table>
					<tr>
						...
						<td>
							...
							ic_select_page(
								'shopping_cart_settings[shopping_cart_page]',
								__( 'Select Cart Page', 'ecommerce-product-catalog' ),
								$shopping_cart_settings['shopping_cart_page'],
								true,
								false,
								1,
					...
					<tr>
						...
						<td>
							...
							ic_select_page(
								'shopping_cart_settings[cart_submit_page]',
								__( 'Select Submit Page', 'ecommerce-product-catalog' ),
								$shopping_cart_settings['cart_submit_page'],
								true,
								false,
								1,
					...
					<tr>
						...
						<td>
							...
							ic_select_page(
								'shopping_cart_settings[thank_you_page]',
								__( 'Select Thank You Page', 'ecommerce-product-catalog' ),
								$shopping_cart_settings['thank_you_page'],
								true,
								false,
								1,
				...
				<?php
					implecode_settings_radio(
						__( 'Remember fields by', 'ecommerce-product-catalog' ),
						'shopping_cart_settings[form_registration]',
						$shopping_cart_settings['form_registration'],
						array(
						...
						1,
						__( 'If you select user account you will be able to select a page to become user account with the order history.', 'ecommerce-product-catalog' )
					);
...
function ic_shopping_cart_settings_validation( $new_value, $old_value ) {
	...
	} elseif ( empty( $old_value['shopping_cart_page'] ) && ! empty( $new_value['shopping_cart_page'] ) ) {
		ic_add_shopping_cart_page_shortcode( intval( $new_value['shopping_cart_page'] ), 'shopping_cart' );
	}
	...
	} elseif ( empty( $old_value['cart_submit_page'] ) && ! empty( $new_value['cart_submit_page'] ) ) {
		ic_add_shopping_cart_page_shortcode( intval( $new_value['cart_submit_page'] ), 'cart_submit_form' );
	}
	...
	} elseif ( empty( $old_value['thank_you_page'] ) && ! empty( $new_value['thank_you_page'] ) ) {
		ic_add_shopping_cart_page_shortcode( intval( $new_value['thank_you_page'] ), 'success_page' );
	}
...
function ic_add_shopping_cart_page_shortcode( $page_id, $shortcode = '' ) {
	...
	} elseif ( ! has_shortcode( $post->post_content, $shortcode ) && ! ic_string_contains( $post->post_content, '[' ) ) {
