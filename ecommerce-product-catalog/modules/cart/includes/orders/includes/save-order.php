...
class ic_orders_save {
	...
	function save( $message, $customer_email, $redirect, $contact, $pre_name ) {
		if ( $pre_name == 'cart_' ) {
			...
			if ( function_exists( 'ic_count_shipping_cost_payment' ) ) {
				$shipping_summary['price'] = ic_count_shipping_cost_payment( 0, $pre_name );
				if ( ! empty( $order_id ) && ! is_wp_error( $order_id ) ) {
					$labels                     = ic_get_order_shipping_labels( $pre_name );
					$shipping_summary['labels'] = $labels;
					if ( ! empty( $shipping_summary['labels'] ) ) {
	...
	function shipping_summary( $order_id ) {
		...
		if ( ! empty( $shipping_summary ) && ! empty( $shipping_summary['labels'] ) ) {
			?>
			<tr>
				<td><?php echo __( 'Shipping', 'ecommerce-product-catalog' ) . ' (' . $shipping_summary['labels'] . ')'; ?></td>
				<td><?php echo ic_shipping_price_format( $shipping_summary['price'] ); ?></td>
			</tr>
			<?php
		}
	}
	...
	function trans_id( $content ) {
		...
		$rand     = rand( 1, 9999 );
		$content .= '<tr class="trans-id-row" style="display:none"><td><input type="hidden" value="' . $time . $rand . '" name="trans_id"></td></tr>';
	...
	function name( $order_id ) {
		...
		} elseif ( is_numeric( $payment_details['date'] ) ) {
